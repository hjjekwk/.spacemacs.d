...
layers configuration. You are free to put any user code."
  (global-set-key (kbd "C-;") 'turn-on-skk-mode)
  ...
  (let ((user-prodigy-dir (expand-file-name "~/.prodigy.d")))
    (when (file-exists-p user-prodigy-dir)
      (mapc 'load (directory-files user-prodigy-dir t "^[^#].*el$"))))

  ;; Settings for currently logged in user
  (setq user-settings-dir "~/.user.emacs.d")
  (add-to-list 'load-path user-settings-dir)

  ;; Conclude init by setting up specifics for the current user
  (let ((user-settings-init-file (concat user-settings-dir "/init.el")))
    (when (file-exists-p user-settings-init-file)
      (load user-settings-init-file)))
  )
